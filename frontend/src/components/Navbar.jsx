...
export default function Navbar() {
  ...
  useEffect(() => {
    fetch(`https://venuex-bmu7.onrender.com/navbar`, {
      method: 'GET',
      credentials: 'include',
    })
      .then(res => {
  ...
  return (
    <nav className="flex items-center justify-between px-8 md:px-16 py-5 shadow-sm bg-white/70 backdrop-blur-md sticky top-0 z-50">
      <Link to="/">
        <h1 className="text-2xl font-extrabold text-blue-600 tracking-tight">VenueX</h1>
      </Link>
      <div className="space-x-8 hidden md:flex">
        <Link
          to={role == 'admin' ? '/add' : '/venues'}
          className="text-gray-700 hover:text-blue-600 transition"
        >
          {role === 'admin' ? `Add Venue` : 'View Sites'}
        </Link>
        <Link
          to={role == 'admin' ? '/listed' : '/booked'}
          className="text-gray-700 hover:text-blue-600 transition"
        >
